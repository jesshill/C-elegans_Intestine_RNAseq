...
#BiocManager::install(c("DESeq2", "apeglm", "topGO"))

#install.packages(c("corrplot", "pheatmap", "RColorBrewer", "calibrate", 'ashr', "tidyverse"))
```
...
# clec-47 = WBGene00011668
reslfc_CeMbio.yes_CeMbio.no
reslfc_CeMbio.yes_OP50.yes
reslfc_CeMbio.no_OP50.no
reslfc_OP50.yes_OP50.no 

plotCounts(dds, gene=which(rownames(reslfc_CeMbio.no_OP50.no) == "WBGene00011668"), intgroup="group", pch = 20, cex = 3)
```
...
```{r}
dim(Up_CeMbio.yes_CeMbio.no)
dim(Down_CeMbio.yes_CeMbio.no)
dim(Up_CeMbio.yes_OP50.yes)
dim(Down_CeMbio.yes_OP50.yes)
dim(Up_CeMbio.no_OP50.no)
dim(Down_CeMbio.no_OP50.no)
dim(Up_OP50.yes_OP50.no)
dim(Down_OP50.yes_OP50.no)
...
dim(changing_exposure)
length(unique(rownames(changing_exposure)))

dim(changing_diet)
length(unique(rownames(changing_diet)))
...
WBgeneID <- rownames(Up_CeMbio.yes_CeMbio.no)
Up_CeMbio.yes_CeMbio.no <- cbind(Up_CeMbio.yes_CeMbio.no, WBgeneID)
Up_CeMbio.yes_CeMbio.no <- as.data.frame(Up_CeMbio.yes_CeMbio.no)

WBgeneID <- rownames(Down_CeMbio.yes_CeMbio.no)
Down_CeMbio.yes_CeMbio.no <- cbind(Down_CeMbio.yes_CeMbio.no, WBgeneID)
Down_CeMbio.yes_CeMbio.no <- as.data.frame(Down_CeMbio.yes_CeMbio.no)

WBgeneID <- rownames(Up_CeMbio.yes_OP50.yes)
Up_CeMbio.yes_OP50.yes <- cbind(Up_CeMbio.yes_OP50.yes, WBgeneID)
Up_CeMbio.yes_OP50.yes <- as.data.frame(Up_CeMbio.yes_OP50.yes)

WBgeneID <- rownames(Down_CeMbio.yes_OP50.yes)
Down_CeMbio.yes_OP50.yes <- cbind(Down_CeMbio.yes_OP50.yes, WBgeneID)
Down_CeMbio.yes_OP50.yes <- as.data.frame(Down_CeMbio.yes_OP50.yes)

WBgeneID <- rownames(Up_CeMbio.no_OP50.no)
Up_CeMbio.no_OP50.no <- cbind(Up_CeMbio.no_OP50.no, WBgeneID)
Up_CeMbio.no_OP50.no <- as.data.frame(Up_CeMbio.no_OP50.no)

WBgeneID <- rownames(Down_CeMbio.no_OP50.no)
Down_CeMbio.no_OP50.no <- cbind(Down_CeMbio.no_OP50.no, WBgeneID)
Down_CeMbio.no_OP50.no <- as.data.frame(Down_CeMbio.no_OP50.no)

WBgeneID <- rownames(Up_OP50.yes_OP50.no)
Up_OP50.yes_OP50.no <- cbind(Up_OP50.yes_OP50.no, WBgeneID)
Up_OP50.yes_OP50.no <- as.data.frame(Up_OP50.yes_OP50.no)

WBgeneID <- rownames(Down_OP50.yes_OP50.no)
Down_OP50.yes_OP50.no <- cbind(Down_OP50.yes_OP50.no, WBgeneID)
Down_OP50.yes_OP50.no <- as.data.frame(Down_OP50.yes_OP50.no)
...
changing_all_genes_data <- biomaRt::getBM(
  mart = paramart,
  filter = c("wbps_gene_id"),
  value = changing_all_genes$WBgeneID,
  attributes = c("wbps_gene_id","wormbase_gseq", "wikigene_name", "entrezgene_id")
) %>% right_join(changing_all_genes, by = c("wbps_gene_id" = "WBgeneID"))

#changing_all_genes_data <- changing_all_genes_data %>% rename(WBgeneID = "wbps_gene_id")

write_csv(changing_all_genes_data, file = "03_output/intestine/changing_all_genes_data_sets.csv")
```
```{r}
paramart <- biomaRt::useMart("parasite_mart", dataset = "wbps_gene", host = "https://parasite.wormbase.org", port = 443)

changing_diet_genes_data <- biomaRt::getBM(
  mart = paramart,
  filter = c("wbps_gene_id"),
  value = changing_diet_genes$WBgeneID,
  attributes = c("wbps_gene_id","wormbase_gseq", "wikigene_name", "entrezgene_id")
) %>% right_join(changing_diet_genes, by = c("wbps_gene_id" = "WBgeneID"))

#changing_diet_genes_data <- changing_diet_genes_data %>% rename(WBgeneID = "wbps_gene_id")

write_csv(changing_diet_genes_data, file = "03_output/intestine/changing_diet_genes_data_sets.csv")
```
```{r}
paramart <- biomaRt::useMart("parasite_mart", dataset = "wbps_gene", host = "https://parasite.wormbase.org", port = 443)

changing_exposure_genes_data <- biomaRt::getBM(
  mart = paramart,
  filter = c("wbps_gene_id"),
  value = changing_exposure_genes$WBgeneID,
  attributes = c("wbps_gene_id","wormbase_gseq", "wikigene_name", "entrezgene_id")
) %>% right_join(changing_exposure_genes, by = c("wbps_gene_id" = "WBgeneID"))

#changing_exposure_genes_data <- changing_exposure_genes_data %>% rename(WBgeneID = "wbps_gene_id")

write_csv(changing_exposure_genes_data, file = "03_output/intestine/changing_exposure_genes_data_sets.csv")
```

```{r}
paramart <- biomaRt::useMart("parasite_mart", dataset = "wbps_gene", host = "https://parasite.wormbase.org", port = 443)

sign_CeMbio.yes_CeMbio.no_data <- biomaRt::getBM(
  mart = paramart,
  filter = c("wbps_gene_id"),
  value = sign_CeMbio.yes_CeMbio.no$WBgeneID,
  attributes = c("wbps_gene_id","wormbase_gseq", "wikigene_name", "entrezgene_id")
) %>% right_join(sign_CeMbio.yes_CeMbio.no, by = c("wbps_gene_id" = "WBgeneID"))

#sign_CeMbio.yes_CeMbio.no_data <- sign_CeMbio.yes_CeMbio.no_data %>% rename(WBgeneID = "wbps_gene_id")

write_csv(sign_CeMbio.yes_CeMbio.no_data, file = "03_output/intestine/sign_CeMbio.yes_CeMbio.no_data_sets.csv")
```
```{r}
paramart <- biomaRt::useMart("parasite_mart", dataset = "wbps_gene", host = "https://parasite.wormbase.org", port = 443)

sign_CeMbio.yes_OP50.yes_data <- biomaRt::getBM(
  mart = paramart,
  filter = c("wbps_gene_id"),
  value = sign_CeMbio.yes_OP50.yes$WBgeneID,
  attributes = c("wbps_gene_id","wormbase_gseq", "wikigene_name", "entrezgene_id")
) %>% right_join(sign_CeMbio.yes_OP50.yes, by = c("wbps_gene_id" = "WBgeneID"))

#sign_CeMbio.yes_OP50.yes_data <- sign_CeMbio.yes_OP50.yes_data %>% rename(WBgeneID = "wbps_gene_id")

write_csv(sign_CeMbio.yes_OP50.yes_data, file = "03_output/intestine/sign_CeMbio.yes_OP50.yes_data_sets.csv")
```
```{r}
paramart <- biomaRt::useMart("parasite_mart", dataset = "wbps_gene", host = "https://parasite.wormbase.org", port = 443)

sign_CeMbio.no_OP50.no_data <- biomaRt::getBM(
  mart = paramart,
  filter = c("wbps_gene_id"),
  value = sign_CeMbio.no_OP50.no$WBgeneID,
  attributes = c("wbps_gene_id","wormbase_gseq", "wikigene_name", "entrezgene_id")
) %>% right_join(sign_CeMbio.no_OP50.no, by = c("wbps_gene_id" = "WBgeneID"))

#sign_CeMbio.no_OP50.no_data <- sign_CeMbio.no_OP50.no_data %>% rename(WBgeneID = "wbps_gene_id")

write_csv(sign_CeMbio.no_OP50.no_data, file = "03_output/intestine/sign_CeMbio.no_OP50.no_data_sets.csv")
```
```{r}
paramart <- biomaRt::useMart("parasite_mart", dataset = "wbps_gene", host = "https://parasite.wormbase.org", port = 443)

sign_OP50.yes_OP50.no_data <- biomaRt::getBM(
  mart = paramart,
  filter = c("wbps_gene_id"),
  value = sign_OP50.yes_OP50.no$WBgeneID,
  attributes = c("wbps_gene_id","wormbase_gseq", "wikigene_name", "entrezgene_id")
) %>% right_join(sign_OP50.yes_OP50.no, by = c("wbps_gene_id" = "WBgeneID"))

#sign_OP50.yes_OP50.no_data <- sign_OP50.yes_OP50.no_data %>% rename(WBgeneID = "wbps_gene_id")

write_csv(sign_OP50.yes_OP50.no_data, file = "03_output/intestine/sign_OP50.yes_OP50.no_data_sets.csv")
```

```{r}
paramart <- biomaRt::useMart("parasite_mart", dataset = "wbps_gene", host = "https://parasite.wormbase.org", port = 443)

Up_CeMbio.yes_CeMbio.no_data <- biomaRt::getBM(
  mart = paramart,
  filter = c("wbps_gene_id"),
  value = Up_CeMbio.yes_CeMbio.no$WBgeneID,
  attributes = c("wbps_gene_id","wormbase_gseq", "wikigene_name", "entrezgene_id")
) %>% right_join(Up_CeMbio.yes_CeMbio.no, by = c("wbps_gene_id" = "WBgeneID"))

#Up_CeMbio.yes_CeMbio.no_data <- Up_CeMbio.yes_CeMbio.no_data %>% rename(WBgeneID = "wbps_gene_id")

write_csv(Up_CeMbio.yes_CeMbio.no_data, file = "03_output/intestine/Up_CeMbio.yes_CeMbio.no_data_sets.csv")
```
```{r}
paramart <- biomaRt::useMart("parasite_mart", dataset = "wbps_gene", host = "https://parasite.wormbase.org", port = 443)

Down_CeMbio.yes_CeMbio.no_data <- biomaRt::getBM(
  mart = paramart,
  filter = c("wbps_gene_id"),
  value = Down_CeMbio.yes_CeMbio.no$WBgeneID,
  attributes = c("wbps_gene_id","wormbase_gseq", "wikigene_name", "entrezgene_id")
) %>% right_join(Down_CeMbio.yes_CeMbio.no, by = c("wbps_gene_id" = "WBgeneID"))

#Down_CeMbio.yes_CeMbio.no_data <- Down_CeMbio.yes_CeMbio.no_data %>% rename(WBgeneID = "wbps_gene_id")

write_csv(Down_CeMbio.yes_CeMbio.no_data, file = "03_output/intestine/Down_CeMbio.yes_CeMbio.no_data_sets.csv")
```
```{r}
paramart <- biomaRt::useMart("parasite_mart", dataset = "wbps_gene", host = "https://parasite.wormbase.org", port = 443)

Up_CeMbio.yes_OP50.yes_data <- biomaRt::getBM(
  mart = paramart,
  filter = c("wbps_gene_id"),
  value = Up_CeMbio.yes_OP50.yes$WBgeneID,
  attributes = c("wbps_gene_id","wormbase_gseq", "wikigene_name", "entrezgene_id")
) %>% right_join(Up_CeMbio.yes_OP50.yes, by = c("wbps_gene_id" = "WBgeneID"))

#Up_CeMbio.yes_OP50.yes_data <- Up_CeMbio.yes_OP50.yes_data %>% rename(WBgeneID = "wbps_gene_id")

write_csv(Up_CeMbio.yes_OP50.yes_data, file = "03_output/intestine/Up_CeMbio.yes_OP50.yes_data_sets.csv")
```
```{r}
paramart <- biomaRt::useMart("parasite_mart", dataset = "wbps_gene", host = "https://parasite.wormbase.org", port = 443)

Down_CeMbio.yes_OP50.yes_data <- biomaRt::getBM(
  mart = paramart,
  filter = c("wbps_gene_id"),
  value = Down_CeMbio.yes_OP50.yes$WBgeneID,
  attributes = c("wbps_gene_id","wormbase_gseq", "wikigene_name", "entrezgene_id")
) %>% right_join(Down_CeMbio.yes_OP50.yes, by = c("wbps_gene_id" = "WBgeneID"))

#Down_CeMbio.yes_OP50.yes_data <- Down_CeMbio.yes_OP50.yes_data %>% rename(WBgeneID = "wbps_gene_id")

write_csv(Down_CeMbio.yes_OP50.yes_data, file = "03_output/intestine/Down_CeMbio.yes_OP50.yes_data_sets.csv")
```
```{r}
paramart <- biomaRt::useMart("parasite_mart", dataset = "wbps_gene", host = "https://parasite.wormbase.org", port = 443)

Up_CeMbio.no_OP50.no_data <- biomaRt::getBM(
  mart = paramart,
  filter = c("wbps_gene_id"),
  value = Up_CeMbio.no_OP50.no$WBgeneID,
  attributes = c("wbps_gene_id","wormbase_gseq", "wikigene_name", "entrezgene_id")
) %>% right_join(Up_CeMbio.no_OP50.no, by = c("wbps_gene_id" = "WBgeneID"))

#Up_CeMbio.no_OP50.no_data <- Up_CeMbio.no_OP50.no_data %>% rename(WBgeneID = "wbps_gene_id")

write_csv(Up_CeMbio.no_OP50.no_data, file = "03_output/intestine/Up_CeMbio.no_OP50.no_data_sets.csv")
```
```{r}
paramart <- biomaRt::useMart("parasite_mart", dataset = "wbps_gene", host = "https://parasite.wormbase.org", port = 443)

Down_CeMbio.no_OP50.no_data <- biomaRt::getBM(
  mart = paramart,
  filter = c("wbps_gene_id"),
  value = Down_CeMbio.no_OP50.no$WBgeneID,
  attributes = c("wbps_gene_id","wormbase_gseq", "wikigene_name", "entrezgene_id")
) %>% right_join(Down_CeMbio.no_OP50.no, by = c("wbps_gene_id" = "WBgeneID"))

#Down_CeMbio.no_OP50.no_data <- Down_CeMbio.no_OP50.no_data %>% rename(WBgeneID = "wbps_gene_id")

write_csv(Down_CeMbio.no_OP50.no_data, file = "03_output/intestine/Down_CeMbio.no_OP50.no_data_sets.csv")
```
```{r}
paramart <- biomaRt::useMart("parasite_mart", dataset = "wbps_gene", host = "https://parasite.wormbase.org", port = 443)

Up_OP50.yes_OP50.no_data <- biomaRt::getBM(
  mart = paramart,
  filter = c("wbps_gene_id"),
  value = Up_OP50.yes_OP50.no$WBgeneID,
  attributes = c("wbps_gene_id","wormbase_gseq", "wikigene_name", "entrezgene_id")
) %>% right_join(Up_OP50.yes_OP50.no, by = c("wbps_gene_id" = "WBgeneID"))

#Up_OP50.yes_OP50.no_data <- Up_OP50.yes_OP50.no_data %>% rename(WBgeneID = "wbps_gene_id")

write_csv(Up_OP50.yes_OP50.no_data, file = "03_output/intestine/Up_OP50.yes_OP50.no_data_sets.csv")
```
```{r}
paramart <- biomaRt::useMart("parasite_mart", dataset = "wbps_gene", host = "https://parasite.wormbase.org", port = 443)

Down_OP50.yes_OP50.no_data <- biomaRt::getBM(
  mart = paramart,
  filter = c("wbps_gene_id"),
  value = Down_OP50.yes_OP50.no$WBgeneID,
  attributes = c("wbps_gene_id","wormbase_gseq", "wikigene_name", "entrezgene_id")
) %>% right_join(Down_OP50.yes_OP50.no, by = c("wbps_gene_id" = "WBgeneID"))

#Down_OP50.yes_OP50.no_data <- Down_OP50.yes_OP50.no_data %>% rename(WBgeneID = "wbps_gene_id")

write_csv(Down_OP50.yes_OP50.no_data, file = "03_output/intestine/Down_OP50.yes_OP50.no_data_sets.csv")
```
...
```
```{r}
changing_all_genes <- read.csv('03_output/intestine/changing_all_genes_data_sets.csv', header = TRUE)
changing_diet_genes <- read.csv('03_output/intestine/changing_diet_genes_data_sets.csv', header = TRUE)
changing_exposure_genes <- read.csv('03_output/intestine/changing_exposure_genes_data_sets.csv', header=TRUE)

###############################
sign_CeMbio_yes_CeMbio_no <- read.csv('03_output/intestine/sign_CeMbio.yes_CeMbio.no_data_sets.csv', header = TRUE)
sign_CeMbio_yes_OP50_yes <- read.csv('03_output/intestine/sign_CeMbio.yes_OP50.yes_data_sets.csv', header = TRUE)
sign_CeMbio_no_OP50_no <- read.csv('03_output/intestine/sign_CeMbio.no_OP50.no_data_sets.csv', header = TRUE)
sign_OP50_yes_OP50_no <- read.csv('03_output/intestine/sign_OP50.yes_OP50.no_data_sets.csv', header = TRUE)

###############################
Up_CeMbio_yes_CeMbio_no <- read.csv('03_output/intestine/Up_CeMbio.yes_CeMbio.no_data_sets.csv', header = TRUE)
Down_CeMbio_yes_CeMbio_no <- read.csv('03_output/intestine/Down_CeMbio.yes_CeMbio.no_data_sets.csv', header = TRUE)
Up_CeMbio_yes_OP50_yes <- read.csv('03_output/intestine/Up_CeMbio.yes_OP50.yes_data_sets.csv', header = TRUE)
Down_CeMbio_yes_OP50_yes <- read.csv('03_output/intestine/Down_CeMbio.yes_OP50.yes_data_sets.csv', header = TRUE)
Up_CeMbio_no_OP50_no <- read.csv('03_output/intestine/Up_CeMbio.no_OP50.no_data_sets.csv', header = TRUE)
Down_CeMbio_no_OP50_no <- read.csv('03_output/intestine/Down_CeMbio.no_OP50.no_data_sets.csv', header = TRUE)
Up_OP50_yes_OP50_no <- read.csv('03_output/intestine/Up_OP50.yes_OP50.no_data_sets.csv', header = TRUE)
Down_OP50_yes_OP50_no <- read.csv('03_output/intestine/Down_OP50.yes_OP50.no_data_sets.csv', header = TRUE)
```
GO classification
```
# ont = "CC"
...
gene_ids <- as.character(sign_CeMbio_yes_CeMbio_no$entrezgene_id)

go_class1 <- groupGO(gene = gene_ids, OrgDb = 'org.Ce.eg.db', ont = "BP", level = 1, readable = TRUE)

head(go_class1)
```
GO over-representation analysis
```{r}
ego1 <- enrichGO(sign_CeMbio_yes_CeMbio_no$entrezgene_id, 'org.Ce.eg.db', ont = "BP", pvalueCutoff = 0.05, pAdjustMethod = "BH", qvalueCutoff = 0.2, readable = TRUE)
# save plot as a .png
png("03_output/intestine/GO_overRep_sign_CeMbio_yes_CeMbio_no.png")
goplot(ego1)
dev.off()

ego2 <- enrichGO(sign_CeMbio_yes_OP50_yes$entrezgene_id, 'org.Ce.eg.db', ont = "BP", pvalueCutoff = 0.05, pAdjustMethod = "BH", qvalueCutoff = 0.2, readable = TRUE)
# save plot as a .png
png("03_output/intestine/GO_overRep_sign_CeMbio_yes_OP50_yes.png")
goplot(ego2)
dev.off()

ego3 <- enrichGO(sign_CeMbio_no_OP50_no$entrezgene_id, 'org.Ce.eg.db', ont = "BP", pvalueCutoff = 0.05, pAdjustMethod = "BH", qvalueCutoff = 0.2, readable = TRUE)
# save plot as a .png
png("03_output/intestine/GO_overRep_sign_CeMbio_no_OP50_no.png")
goplot(ego3)
dev.off()

ego4 <- enrichGO(sign_OP50_yes_OP50_no$entrezgene_id, 'org.Ce.eg.db', ont = "BP", pvalueCutoff = 0.05, pAdjustMethod = "BH", qvalueCutoff = 0.2, readable = TRUE)
png("03_output/intestine/GO_overRep_sign_OP50_yes_OP50_no.png")
goplot(ego4)
dev.off()
```
GO gene set enrichment analysis <-- NOT WORKING!
```
gene_list <- as.character(sign_CeMbio_yes_CeMbio_no$entrezgene_id)
gene_list = sort(gene_list, decreasing = TRUE)
  
gse1 <- gseGO(gene_list, 'org.Ce.eg.db', ont = "MF", minGSSize = 100, maxGSSize = 500, pvalueCutoff = 0.05)
...
p1 <- dotplot(ego1, showCategory = 10, font.size=14)
#p2 <- dotplot(ego2, showCategory = 10, font.size=14)
p3 <- dotplot(ego3, showCategory = 10, font.size=14)
p4 <- dotplot(ego4, showCategory = 10, font.size=14)

png("03_output/intestine/cowplot_sign_CeMbio_yes_CeMbio_no.png")
cowplot::plot_grid(p1, labels="Response to PA14 in CeMbio worms")
dev.off()

png("03_output/intestine/cowplot_sign_CeMbio_no_OP50_no.png")
cowplot::plot_grid(p3, labels="CeMbio vs OP50 fed worms")
dev.off()

png("03_output/intestine/cowplot_sign_OP50_yes_OP50_no.png")
cowplot::plot_grid(p4, labels="Response to PA14 in OP50 worms")
dev.off()
```

```{r}
Up_CeMbio_yes_CeMbio_no
Down_CeMbio_yes_CeMbio_no
Up_CeMbio_yes_OP50_yes
Down_CeMbio_yes_OP50_yes
Up_CeMbio_no_OP50_no
Down_CeMbio_no_OP50_no
Up_OP50_yes_OP50_no
Down_OP50_yes_OP50_no


ego5 <- enrichGO(Up_CeMbio_yes_CeMbio_no$entrezgene_id, 'org.Ce.eg.db', ont = "BP", pvalueCutoff = 0.05, pAdjustMethod = "BH", qvalueCutoff = 0.2, readable = TRUE)
png("03_output/intestine/GO_overRep_Up_CeMbio_yes_CeMbio_no.png")
goplot(ego5)
dev.off()

ego6 <- enrichGO(Down_CeMbio_yes_CeMbio_no$entrezgene_id, 'org.Ce.eg.db', ont = "BP", pvalueCutoff = 0.05, pAdjustMethod = "BH", qvalueCutoff = 0.2, readable = TRUE)
png("03_output/intestine/GO_overRep_Down_CeMbio_yes_CeMbio_no.png")
goplot(ego6)
dev.off()

ego7 <- enrichGO(Up_CeMbio_yes_OP50_yes$entrezgene_id, 'org.Ce.eg.db', ont = "BP", pvalueCutoff = 0.05, pAdjustMethod = "BH", qvalueCutoff = 0.2, readable = TRUE)
png("03_output/intestine/GO_overRep_Up_CeMbio_yes_OP50_yes.png")
goplot(ego7)
dev.off()

ego8 <- enrichGO(Down_CeMbio_yes_OP50_yes$entrezgene_id, 'org.Ce.eg.db', ont = "BP", pvalueCutoff = 0.05, pAdjustMethod = "BH", qvalueCutoff = 0.2, readable = TRUE)
png("03_output/intestine/GO_overRep_Down_CeMbio_yes_OP50_yes.png")
goplot(ego8)
dev.off()

ego9 <- enrichGO(Up_CeMbio_no_OP50_no$entrezgene_id, 'org.Ce.eg.db', ont = "BP", pvalueCutoff = 0.05, pAdjustMethod = "BH", qvalueCutoff = 0.2, readable = TRUE)
png("03_output/intestine/GO_overRep_Up_CeMbio_no_OP50_no.png")
goplot(ego9)
dev.off()

ego10 <- enrichGO(Down_CeMbio_no_OP50_no$entrezgene_id, 'org.Ce.eg.db', ont = "BP", pvalueCutoff = 0.05, pAdjustMethod = "BH", qvalueCutoff = 0.2, readable = TRUE)
png("03_output/intestine/GO_overRep_Down_CeMbio_no_OP50_no.png")
goplot(ego10)
dev.off()

ego11 <- enrichGO(Up_OP50_yes_OP50_no$entrezgene_id, 'org.Ce.eg.db', ont = "BP", pvalueCutoff = 0.05, pAdjustMethod = "BH", qvalueCutoff = 0.2, readable = TRUE)
png("03_output/intestine/GO_overRep_Up_OP50_yes_OP50_no.png")
goplot(ego11)
dev.off()

ego12 <- enrichGO(Down_OP50_yes_OP50_no$entrezgene_id, 'org.Ce.eg.db', ont = "BP", pvalueCutoff = 0.05, pAdjustMethod = "BH", qvalueCutoff = 0.2, readable = TRUE)
png("03_output/intestine/GO_overRep_Down_OP50_yes_OP50_no.png")
goplot(ego12)
dev.off()

p5 <- dotplot(ego5, showCategory = 10, font.size=14)
p6 <- dotplot(ego6, showCategory = 10, font.size=14)
p7 <- dotplot(ego7, showCategory = 10, font.size=14)
p8 <- dotplot(ego8, showCategory = 10, font.size=14)
p9 <- dotplot(ego9, showCategory = 10, font.size=14)
p10 <- dotplot(ego10, showCategory = 10, font.size=14)
p11 <- dotplot(ego11, showCategory = 10, font.size=14)
p12 <- dotplot(ego12, showCategory = 10, font.size=14)

png("03_output/intestine/cowplot_Up_CeMbio_yes_CeMbio_no.png")
cowplot::plot_grid(p5, labels="Response to PA14 in CeMbio (up genes)")
dev.off()

png("03_output/intestine/cowplot_Down_CeMbio_yes_CeMbio_no.png")
cowplot::plot_grid(p6, labels="Response to PA14 in CeMbio (down genes)")
dev.off()

png("03_output/intestine/cowplot_Up_CeMbio_yes_OP50_yes.png")
cowplot::plot_grid(p7, labels="CeMbio vs OP50 during challenge (up genes)")
dev.off()

png("03_output/intestine/cowplot_Down_CeMbio_yes_OP50_yes.png")
cowplot::plot_grid(p8, labels="CeMbio vs OP50 during challenge (down genes)")
dev.off()

png("03_output/intestine/cowplot_Up_CeMbio_no_OP50_no.png")
cowplot::plot_grid(p9, labels="CeMbio vs OP50 (up genes)")
dev.off()

png("03_output/intestine/cowplot_Down_CeMbio_no_OP50_no.png")
cowplot::plot_grid(p10, labels="CeMbio vs OP50 (down genes)")
dev.off()

png("03_output/intestine/cowplot_Up_OP50_yes_OP50_no.png")
cowplot::plot_grid(p11, labels="Response to PA14 in OP50 (up genes)")
dev.off()

png("03_output/intestine/cowplot_Down_OP50_yes_OP50_no.png")
cowplot::plot_grid(p12, labels="Response to PA14 in OP50 (down genes)")
dev.off()
```
...
```
```{python}
changing_all_genes=pd.read_csv('03_output/intestine/changing_all_genes_data_sets.csv')
changing_diet_genes=pd.read_csv('03_output/intestine/changing_diet_genes_data_sets.csv')
changing_exposure_genes=pd.read_csv('03_output/intestine/changing_exposure_genes_data_sets.csv')

###############################
sign_CeMbio_yes_CeMbio_no=pd.read_csv('03_output/intestine/sign_CeMbio.yes_CeMbio.no_data_sets.csv')
sign_CeMbio_yes_OP50_yes=pd.read_csv('03_output/intestine/sign_CeMbio.yes_OP50.yes_data_sets.csv')
sign_CeMbio_no_OP50_no=pd.read_csv('03_output/intestine/sign_CeMbio.no_OP50.no_data_sets.csv')
sign_OP50_yes_OP50_no=pd.read_csv('03_output/intestine/sign_OP50.yes_OP50.no_data_sets.csv')

###############################
Up_CeMbio_yes_CeMbio_no=pd.read_csv('03_output/intestine/Up_CeMbio.yes_CeMbio.no_data_sets.csv')
Down_CeMbio_yes_CeMbio_no=pd.read_csv('03_output/intestine/Down_CeMbio.yes_CeMbio.no_data_sets.csv')
Up_CeMbio_yes_OP50_yes=pd.read_csv('03_output/intestine/Up_CeMbio.yes_OP50.yes_data_sets.csv')
Down_CeMbio_yes_OP50_yes=pd.read_csv('03_output/intestine/Down_CeMbio.yes_OP50.yes_data_sets.csv')
Up_CeMbio_no_OP50_no=pd.read_csv('03_output/intestine/Up_CeMbio.no_OP50.no_data_sets.csv')
Down_CeMbio_no_OP50_no=pd.read_csv('03_output/intestine/Down_CeMbio.no_OP50.no_data_sets.csv')
Up_OP50_yes_OP50_no=pd.read_csv('03_output/intestine/Up_OP50.yes_OP50.no_data_sets.csv')
Down_OP50_yes_OP50_no=pd.read_csv('03_output/intestine/Down_OP50.yes_OP50.no_data_sets.csv')
```


```{python}
a = sign_OP50_yes_OP50_no['wbps_gene_id']
b = sign_CeMbio_yes_CeMbio_no['wbps_gene_id']
c = sign_CeMbio_no_OP50_no['wbps_gene_id']
d = sign_CeMbio_yes_OP50_yes['wbps_gene_id']
all_background_set = pd.DataFrame(pd.concat([a, b, c, d], ignore_index=True))
all_background_set.to_csv('03_output/intestine/all_background_set.csv', index=False)

#one = sign_OP50_yes_OP50_no['wbps_gene_id']
#two = sign_CeMbio_yes_CeMbio_no['wbps_gene_id']
...
#three = sign_CeMbio_no_OP50_no['wbps_gene_id']
#four = sign_CeMbio_yes_OP50_yes['wbps_gene_id']
#Diet_background_set = pd.DataFrame(pd.concat([three, four], ignore_index=True))
...
for analysis, dictionary in frames.items():
  result[analysis] = ea.enrichment_analysis(Down_OP50_yes_OP50_no.wbps_gene_id, dictionary, show=True, alpha=10**-1)
...
# save figure
plt.savefig('03_output/intestine/Down_OP50_yes_OP50_no_Enrichment_Results.svg', bbox_inches='tight')  

# save the results from GO to a csv
results = result["go"]
results.to_csv('03_output/intestine/Results-Down_OP50_yes_OP50_no-comparison.csv', index=False)
```
...
result_background = {}
for analysis, dictionary in frames.items():
  result_background[analysis] = ea.enrichment_analysis(Down_CeMbio_yes_CeMbio_no.wbps_gene_id, dictionary[dictionary.wbid.isin(all_background_set.wbps_gene_id)], show=True, alpha=10**-1)
...
# go through the results hash:
for t, r in result_background.items():
    # calculate the negative log of the Q-values and store
    r['logQ'] = -r['Q value'].apply(np.log10)
    # remove np.infinites with np.nan
    r.logQ.replace([np.inf], np.nan, inplace=True)
...
# save figure
plt.savefig('03_output/intestine/Down_CeMbio_yes_CeMbio_no_Enrichment_Results_background.svg', bbox_inches='tight')  

# save the results from GO to a csv
results_background = result_background["go"]
results_background.to_csv('03_output/intestine/Results-Down_CeMbio_yes_CeMbio_no-comparison_background.csv', index=False)
```
...
sign_CeMbio_yes_CeMbio_no_comparison %>% top_n(10, logQ) %>% arrange(logQ) %>%
...
sign_CeMbio.yes_CeMbio.no <- read.csv2("03_output/intestine/sign_CeMbio.yes_CeMbio.no_data_sets.csv", header = TRUE, sep = ",")
sign_CeMbio.yes_OP50.yes <- read.csv2("03_output/intestine/sign_CeMbio.yes_OP50.yes_data_sets.csv", header = TRUE, sep = ",")
sign_CeMbio.no_OP50.no <- read.csv2("03_output/intestine/sign_CeMbio.no_OP50.no_data_sets.csv", header = TRUE, sep = ",")
sign_OP50.yes_OP50.no <- read.csv2("03_output/intestine/sign_OP50.yes_OP50.no_data_sets.csv", header = TRUE, sep = ",")
```
```{r}
lt2 <- list(changing_all_genes = changing_all_genes$wbps_gene_id, 
           changing_diet_genes = changing_diet_genes$wbps_gene_id, 
           changing_exposure_genes = changing_exposure_genes$wbps_gene_id)

lt <- list(CeMbio.yes_CeMbio.no = sign_CeMbio.yes_CeMbio.no$wbps_gene_id, 
           CeMbio.yes_OP50.yes = sign_CeMbio.yes_OP50.yes$wbps_gene_id,
           CeMbio.no_OP50.no = sign_CeMbio.no_OP50.no$wbps_gene_id,
           OP50.yes_OP50.no = sign_OP50.yes_OP50.no$wbps_gene_id)
...
Volcano plots (need to update - havent fixed yet!)
```{r}
#Identify significantly changing genes
...
png("03_output/intestine/volcano plot - ... .png")
# Reset parameters
par(mfrow=c(1,1))
# Draw the plot
