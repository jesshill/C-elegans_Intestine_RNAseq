...
ggo <- groupGO(gene = Up_CeMbio_yes_CeMbio_no, OrgDb = org.Ce.eg.db, ont = "MF", level = 3, readable = TRUE)
...
# something is not right here in this comparison - need to work on
```{python}
# compare to the generated background set 
result_background = {}
for analysis, dictionary in frames.items():
  result_background[analysis] = ea.enrichment_analysis(changing_exposure_genes.wbps_gene_id, dictionary[dictionary.wbid.isin(PA14_background_set.wbps_gene_id)], show=True, alpha=10**-1)
...
# save figure
plt.savefig('03_output/intestine/changing_exposure_genes_Enrichment_Results_background.svg', bbox_inches='tight')  

# save the results from GO to a csv
results_background = result_background["go"]
results_background.to_csv('03_output/intestine/Results-changing_exposure_genes-comparison_background.csv', index=False)
```
